...
export function useMapPopup() {
  ...
  const buildPopupHTML = (properties: Record<string, any>) => {
    ...
    const formatFlightTime = (secsTotal: number) => {
  ...
  const handleMove = (map: mapboxgl.Map) => (e: mapboxgl.MapLayerMouseEvent) => {
  ...
  const handleEnter = (map: mapboxgl.Map) => () => {
    map.getCanvas().style.cursor = "pointer";
  };

  const handleLeave = (map: mapboxgl.Map) => () => {
  ...
  return { handleMove, handleEnter, handleLeave, cleanup };
}
